// ==UserScript==
// @name         Steam Economy Enhancer-CN
...
(function ($, async) {
    ...
    if (currentPage == PAGE_INVENTORY) {
        ...
        function updateTotals() {
            ...
            if (totalPriceWithFeesOnMarket > 0) {
                totals.innerHTML += `<div><strong>总上架价值 ${formatPrice(totalPriceWithFeesOnMarket)}, 您将收到 ${formatPrice(totalPriceWithoutFeesOnMarket)}。</strong></div>`;
            }
            if (totalScrap > 0) {
        ...
        const sellQueue = async.queue(
            (task, next) => {
                ...
                if (getSettingWithDefault(SETTING_PRICE_MIN_LIST_PRICE) * 100 >= market.getPriceIncludingFees(task.sellPrice)) {
                    logDOM(`${padLeft} - ${itemNameWithAmount} 因价格设置忽略而未上架。`);
                ...
                market.sellItem(
                    ...
                    (error, data) => {
                        ...
                        if (success) {
                            logDOM(`${padLeft} - ${itemNameWithAmount} 以 ${formatPrice(market.getPriceIncludingFees(task.sellPrice) * task.item.amount)} 上架, 您将收到 ${formatPrice(task.sellPrice * task.item.amount)}。`);
                        ...
                        if (message && isRetryMessage(message)) {
                            logDOM(`${padLeft} - ${itemNameWithAmount} 正在重试上架, 原因: ${message.charAt(0).toLowerCase()}${message.slice(1)}`);
            ...
            market.getGooValue(
                ...
                (err, goo) => {
                    ...
                    if (err != ERROR_SUCCESS) {
                        logConsole(`获取 ${itemName} 的宝石值失败`);
                        logDOM(`${padLeft} - ${itemName} 因缺少宝石值而未转为宝石。`);
                    ...
                    market.grindIntoGoo(
                        ...
                        (err) => {
                            if (err != ERROR_SUCCESS) {
                                logConsole(`将 ${itemName} 转为宝石失败`);
                                logDOM(`${padLeft} - ${itemName} 因未知错误而未转为宝石。`);
                            ...
                            logConsole('============================');
                            logConsole(itemName);
                            logConsole(`转为 ${goo.goo_value} 宝石`);
                            logDOM(`${padLeft} - ${itemName} 转为 ${item.goo_value_expected} 宝石。`);
        ...
        function boosterQueueWorker(item, callback) {
            ...
            market.unpackBoosterPack(
                ...
                (err) => {
                    ...
                    if (err != ERROR_SUCCESS) {
                        logConsole(`打开补充包 ${itemName} 失败`);
                        logDOM(`${padLeft} - ${itemName} 未打开。`);
        ...
        function sellSelectedItemsManually() {
            getInventorySelectedMarketableItems((items) => {
                ...
                const dialog = unsafeWindow.ShowDialog('Steam 经济增强器', `<iframe frameBorder="0" height="650" width="900" src="${redirectUrl}"></iframe>`);
                dialog.OnDismiss(() => {
                    items.forEach((item) => {
                        const itemId = item.assetid || item.id;
        ...
        function updateTurnIntoGemsButton() {
            getInventorySelectedGemsItems((items) => {
                ...
                if (items.length == 0) {
                    ...
                    $('.turn_into_gems').show();
                    $('.turn_into_gems > span').
                        text(`将 ${selectedItems}${selectedItems == 1 ? ' 个物品转为宝石' : ' 个物品转为宝石'}`);
                }
            });
        }
            ...
            getInventorySelectedBoosterPackItems((items) => {
                ...
                if (items.length == 0) {
                    ...
                    $('.unpack_selected_booster_packs > span').
                        text(`打开 ${selectedItems}${selectedItems == 1 ? ' 个补充包' : ' 个补充包'}`);
                }
            });
        }
        ...
        async function updateInventorySelection(selectedItem) {
            ...
            // 清除之前添加的内容，避免重复
            $('#listings_group', item_info).remove();
            $('#price_buttons', item_info).remove();
            $('#sell_button', item_info).remove();
        ...
        function updateInventoryUI(isOwnInventory) {
            ...
            $('#see_settings').remove();
            $('#global_action_menu').
                prepend('<span id="see_settings"><a href="javascript:void(0)">⬖ Steam 经济增强器</a></span>');
            ...
            if (showMiscOptions) {
                buttonsHtml += `
                    ...
                    <div class="see_inventory_buttons">
                        <a class="btn_darkblue_white_innerfade btn_medium_wide turn_into_gems" style="display:none"><span>将选中物品转为宝石</span></a>
                        ...
                        <a class="btn_darkblue_white_innerfade btn_medium_wide gem_all_duplicates"><span>将所有重复物品转为宝石</span></a>
                    </div>
                `;
            } else if (TF2) {
    ...
    if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
        ...
        function marketListingsQueueWorker(listing, ignoreErrors, callback) {
            ...
            if (listingUI == null) {
                logConsole(`列表 ${listing.listingid} 在列表中未找到，跳过。`);
                    ...
                    market.getItemOrdersHistogram(
                        ...
                        (errorHistogram, histogram, cachedListings) => {
                            ...
                            $(
                                ...
                                listingUI
                            ).append(` ➤ <span title="这可能是最高收购价格。">${highestBuyOrderPrice
                                }</span>`);
        ...
        function marketOverpricedQueueWorker(item, ignoreErrors, callback) {
            ...
            if (listingUI == null) {
                logConsole(`列表 ${item.listing} 在列表中未找到，跳过。`);
            ...
            marketLists.flatMap(list => list.items).forEach(item => {
                ...
                if (isSellOrder) {
                    ...
                    if (assetInfo.appid === undefined) {
                        logConsole(`跳过列表 ${listingid}（未找到appid）`);
                        return;
                    }
                ...
                logConsole(`跳过物品 ${item.elm.id}（不是买入或卖出订单）`);
            });
        ...
        function updateMarketSelectAllButton() {
            $('.market_listing_buttons').each(function () {
                ...
                if ($('.market_select_item', selectionGroup).length == 0) { // 如果没有可选择的物品，保持为全选。
                    invert = false;
                }
        ...
        function initializeMarketUI() {
            ...
            $('.my_market_header').first().append(`<div class="market_listing_buttons">
                ...
                <a class="item_market_action_button item_market_action_button_green select_overpriced market_listing_button">
                    <span class="item_market_action_button_contents">选择高价物品</span>
                </a>
            </div>`);
            ...
            $('#see_settings').remove();
            $('#global_action_menu').prepend('<span id="see_settings"><a href="javascript:void(0)">⬖ Steam 经济增强器</a></span>');
            $('#see_settings').on('click', '*', () => openSettings());
    ...
    function initializeMarketHistoryUI() {
        ...
        $('#tabContentsMyMarketHistory').observe('childlist subtree', () => {
            ...
            if (controlsDiv.length > 0 && $('#see_page_jump').length === 0) {
                ...
                const input = $('<input type="number" min="1" placeholder="页码" />');
    ...
    if (currentPage == PAGE_TRADEOFFER) {
        ...
        function sumTradeOfferAssets(assets, user) {
            ...
            let totalText = `<strong>唯一物品数量: ${sortable.length}, 价值 ${formatPrice(totalPrice)}<br/><br/></strong>`;
            let totalNumOfItems = 0;
            for (let i = 0; i < sortable.length; i++) {
                totalText += `${sortable[i][1]}x ${sortable[i][0]}<br/>`;
                totalNumOfItems += sortable[i][1];
            }
            totalText += `<br/><strong>总物品数: ${totalNumOfItems}</strong><br/>`;
        ...
        const appendSelectPageButton = () => {
            $('#inventory_displaycontrols').append(`<div class="trade_offer_buttons">
              <a class="item_market_action_button item_market_action_button_green select_all">
                  <span class="item_market_action_button_contents" style="text-transform:none">全选页面物品</span>
              </a>
          </div>`);
    ...
    function openSettings() {
        const price_options = $(`<div id="see_settings_modal">
            <div>
                ...
                <select id="${SETTING_PRICE_ALGORITHM}">
                    <option value="1"${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1 ? 'selected="selected"' : ''}>平均历史价格和最低售价的最大值</option>
                    <option value="2" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2 ? 'selected="selected"' : ''}>最低售价</option>
                    <option value="3" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3 ? 'selected="selected"' : ''}>当前最高收购价或最低售价</option>
                </select>
            </div>
        ...
        unsafeWindow.ShowConfirmDialog('Steam 经济增强器', price_options).done(() => {
